...
mem <- ggplot(data, aes(x = test, y = total.memory, fill = test)) +
        ...
        labs(x = "Tested tools", y = "Total memory usage [GB]") +
        theme_minimal() +
...
time <- ggplot(data, aes(x = test, y = time, fill = test)) +
        geom_bar(stat = "identity") +
        labs(x = "Tested tools", y = "Total time [s]") +
        theme_minimal() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none")
...
#### Particular benchmark analysis reports:


```{r echo=FALSE, message=FALSE, warning=FALSE, results='asis'}


report_list <- list.files(path = file.path(getwd(), 'particular_reports/'), pattern = "*benchmark", full.names = TRUE)

for (file in report_list) {
  
  cat(sprintf("- [%s](%s)\n", sub('.html', '', sub('benchmark_', '', basename(file))), file.path('particular_reports', basename(file))))
}
